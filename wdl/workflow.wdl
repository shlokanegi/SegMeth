...
task runsegmeth {
	...
	command <<<
        ...
        do
            ...
            if [[ $MODKIT_FILENAME =~ \.gz$ ]]; then
                gunzip -c $MODKIT_FILENAME | sort -k1,1 -k2,2n -k3,3n >> ~{sample}.hp${FID}.bed
            else
    ...
    runtime {
        ...
        docker: "quay.io/shnegi/segmeth@sha256:98dab7d210420e39fd2220e028f1f2fc519ebf340cf1b9cd883c1d2aec12cd7a"
        preemptible: 1
    }
}
...
task concatenateBed {
    ...
    output {
        File concatenatedBed1 = "~{sample}.~{region_type}.hp1.bed"
		File concatenatedBed2 = "~{sample}.~{region_type}.hp2.bed"
    }
