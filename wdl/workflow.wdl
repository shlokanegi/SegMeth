...
task runsegmeth {
	...
	command <<<
		...
		do
			...
			## Run CBS
			python3 /opt/scripts/SegMeth-v1.0/cbs_hp.py -file ~{sample}.hp${FID}.chr.bed -o ~{sample}.~{chrom}.hp${FID}.~{suffix} \
			-p ~{pval} -minCG ~{min_cpg} -filt_thresh ~{cpg_vcov} -merge ~{merge} -ut ~{ut} -mt ~{mt} \
			~{extraArgs} -s ~{sample}.hp${FID} -t ~{sample}.target.chr.bed
	...
	runtime {
		...
		docker: "quay.io/shnegi/segmeth@sha256:9786ea902688ed3b6b7ee47faf14db9d199a68afd0f8be533c9ced4209e4bbf5"
		preemptible: 1
	}
}
